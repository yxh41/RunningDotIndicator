...
// ====================================================================
// v2.0.66.7: 常开 Dock 串名探针 (DOCK-NAME-MISMATCH)
// --------------------------------------------------------------------
// 背景：v2.0.66.6 日志事后才开 debug，未捕到 Dock 串名成因；v2.0.66.4 守卫
// （MKLabelDidMoveToSuperviewHook）依赖 didMoveToSuperview + 指针相等判定，
// 在「原地重绑同 icon-view 指针但 app 已换」「didMoveToSuperview 根本不触发」两类
// 复用场景下漏触发 → 残留旧 kMKLabelIconKey 属主指针 → MKLabelToBid 解析到旧属主
// → 外来名字串到 Dock 新槽位。本探针不依赖那两条前提，在 label 显形瞬间比对
// 「tweak 属主(kMKLabelIconKey)」与「几何 Dock 槽位真实 icon-view」指针，不一致即落日志。
// 节流：同一 (owner,slot) 组合 ≥30s 至多 1 条，防刷屏；纯只读 + 记日志，零行为变化。
// ====================================================================
static NSTimeInterval sLastDockMismatchLog = 0;
static __unsafe_unretained UIView *sLastDockMismatchOwner = nil;
static __unsafe_unretained UIView *sLastDockMismatchSlot  = nil;

// 仅 Dock 上下文：向上找 SBDock*/SBIconScrollView 容器，再借 MKIconViewForLabel 几何反解
// 本 label 当前真正所属的 Dock 槽位 icon-view（其下方/水平居中的 SBIconView）。非 Dock 返回 nil。
static UIView *MKDockSlotIconViewForLabel(UIView *label) {
    if (!label) return nil;
    UIView *p = label.superview;
    while (p) {
        NSString *cn = NSStringFromClass([p class]);
        if ([cn hasPrefix:@"SBDock"] || [cn isEqualToString:@"SBIconScrollView"]) {
            return MKIconViewForLabel(label);
        }
        p = p.superview;
    }
    return nil;
...
static void MKDockNameMismatchProbe(UIView *label) {
    if (!label) return;
    @try {
        UIView *owner = objc_getAssociatedObject(label, &kMKLabelIconKey);
        if (!owner) return;                 // 无属主关联 → 非复用串台，跳过
        UIView *slot = MKDockSlotIconViewForLabel(label);
        if (!slot || slot == owner) return; // 几何槽位与 tweak 属主一致 → 正常
        // 进一步比对两槽位解析出的 bid：仅当确为不同 app 才落日志（消除 Dock 横向歧义误判）
        NSString *ownerBid = MKGetCachedBid((SBIconView *)owner);
        NSString *slotBid  = MKGetCachedBid((SBIconView *)slot);
        if (ownerBid.length && slotBid.length && [ownerBid isEqualToString:slotBid]) return;
        NSTimeInterval now = [NSDate timeIntervalSinceReferenceDate];
        if (owner == sLastDockMismatchOwner &&
            slot  == sLastDockMismatchSlot &&
            now - sLastDockMismatchLog < 30.0) return; // 同组合节流
        sLastDockMismatchLog = now;
        sLastDockMismatchOwner = owner;
        sLastDockMismatchSlot  = slot;
        NSString *lblBidKey = (NSString *)objc_getAssociatedObject(label, &kMKLabelBidKey);
        RDLog(@"DOCK-NAME-MISMATCH: label=%p cls=%@ owner=%p ownerBid=%@ slot=%p slotBid=%@ "
              @"labelF=(%.0f,%.0f,%.0f,%.0f) ownerF=(%.0f,%.0f) slotF=(%.0f,%.0f) labelBidKey=%@",
              label, NSStringFromClass([label class]), owner,
              (ownerBid ? ownerBid : @"?"), slot, (slotBid ? slotBid : @"?"),
              label.frame.origin.x, label.frame.origin.y, label.frame.size.width, label.frame.size.height,
              owner.frame.origin.x, owner.frame.origin.y, slot.frame.origin.x, slot.frame.origin.y,
              (lblBidKey ? lblBidKey : @"?"));
    } @catch (NSException *e) {}
}
...
// v2.0.66.7: 常开 Dock 串名探针（定义见 MKLabelToBid 之后）
static UIView *MKDockSlotIconViewForLabel(UIView *label);
static void MKDockNameMismatchProbe(UIView *label);
...
static void MKSetHiddenHook(id self, SEL _cmd, BOOL hidden) {
    @try {
        ...
        if (!hidden) MKDockNameMismatchProbe((UIView *)self); // v2.0.66.7: 常开 Dock 串名探针
...
static void MKSetAlphaHook(id self, SEL _cmd, CGFloat a) {
    @try {
        ...
        if (a > 0.0f) MKDockNameMismatchProbe((UIView *)self); // v2.0.66.7: 常开 Dock 串名探针
        NSString *useBid = nil; BOOL mapOnly = NO;
    ...
    MKSafe(^{
        ...
        if (MKIsFolderIcon((SBIconView *)self)) {
            ...
            if (!indicator) {
                ...
                if (sDebugLog) RDLog(@"FICON-CREATE v2.0.66.8: %@ rep=%@ fixed=%d container=%@ frame=%@", fBid, rep, fixedColor, fContainerCls, NSStringFromCGRect(indicatorFrame));
        ...
        if (!indicator) {
            ...
            if (sDebugLog) RDLog(@"Indicator CREATE v2.0.66.8: %@ shape=%d animate=%d label=%@",
