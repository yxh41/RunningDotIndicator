...
static NSSet *sFolderClosePinViews = nil;  // v2.0.68 方案 D(指针集) + v2.0.69 方案 E(bid 集) 双信号；ARM 时枚举 folder 内层迷你图标，既收指针(SBIconView，抗重父化)又收 bundleID(抗同 App 迷你↔主屏格换对象)。hook 任一命中即钉 alpha=0。
static NSSet *sFolderClosePinBids   = nil;  // v2.0.69 方案 E —— folder 内层 App 的 bundleID 集合(恒定身份，抗重父化/回收重建/迷你↔主屏换对象)；关窗期 hook 直读 [[self icon] applicationBundleID] ∈ 集合 即钉藏。
static NSSet *sFolderCloseArmCacheBids = nil;  // v2.0.70 方案 F —— 关窗「持久 cache」：folder 完整在窗口(SBFolderView -didMoveToWindow: window≠nil) 时枚举的内层 App bundleID 集；关闭时 ARM 直接用(不再现场 BFS, 因关窗子树已在拆致枚举不全 27/50), 绕开 rd_log(117) 的 ARM 漏收 + MKGetCachedBid 缓存丢失(89 条 hasBid=0)。sFolderClosing=NO 时随双集清。
...
static char kMKBornInCloseFolderKey;   // v2.0.75 ①: label 在关窗 folder 内诞生时钉到其属主 SBIconView 的关联键(抗 iOS 重建/重父化)
...
static NSString *MKLabelToBid(UIView *label);                // v2.0.77 前向声明（②探针 MKOpenFixProbe 用）
static void MKOpenFixProbe(UIView *lbl, BOOL inOpenFolder, const char *fn); // v2.0.77 ②探针（定义于 MKIsIconInFolder 之后）
...
//   inOpenFolder=1 → 跳过藏名(应：交 iOS 显示, 名称不闪/指示器正常)
//   inOpenFolder=0 → 仍被强制藏名(漏网: 名称闪一下 / 指示器看不到) → ② 未灭
// 这样下一次日志只要 grep OPENFIX 就能直接判断，不再靠肉眼猜。
static void MKOpenFixProbe(UIView *lbl, BOOL inOpenFolder, const char *fn) {
    if (!sProbeLog) return;
    NSString *b = MKLabelToBid(lbl);
    if (!b.length || !sHiddenBids || ![sHiddenBids containsObject:b]) return;
    UIView *iv = MKIconViewForLabel(lbl);
    UIView *ct = iv ? MKContainerForIconView(iv) : nil;
    NSString *ctc = ct ? NSStringFromClass([ct class]) : @"nil";
    BOOL pf = [ctc isEqualToString:@"SBFolderView"] || [ctc isEqualToString:@"SBFloatyFolderScrollView"];
    if (pf) RDLog(@"OPENFIX: %s inOpenFolder=%d sFO=%d sFC=%d container=%@ bid=%@",
                  fn, (int)inOpenFolder, (int)sFolderOpen, (int)sFolderClosing, ctc, b);
}
...
static void MKSetHiddenHook(id self, SEL _cmd, BOOL hidden) {
    @try {
        ...
        MKOpenFixProbe((UIView *)self, inOpenFolder, __func__);   // v2.0.77 ②探针
...
static void MKLabelDidMoveToWindowHook(id self, SEL _cmd) {
    ...
    @try {
        ...
        if (lbl.window && (lbl.alpha > 0.0f)) {   // v2.0.40: 放宽——覆盖半残态(hidden=YES 但 alpha>0), 旧 !lbl.hidden 把 rd_log(168) CLOSE-TAIL lbl(h=1 a=1.00) 漏过   // v2.0.12: 撤销 v2.0.9 关合窗口内文件夹内 label 让步原生(label 一进 window 即刻强藏, 根除 sub-16ms settle 单帧闪现); 详见 MKSetHiddenHook 同款注释。
            ...
            MKOpenFixProbe(lbl, inOpenFolder, __func__);   // v2.0.77 ②探针
...
static void MKLabelDidMoveToSuperviewHook(id self, SEL _cmd) {
    ...
    // v2.0.75 ① 诞生期锚定: 关窗期间、label 真实属主 SBIconView 位于 folder 缩略图内(MKIsInsideFolderThumb)→ 钉标记到属主。
    // 关联对象随 SBIconView 走、抗 iOS 重建/重父化; setIconLabelAlpha: hook 在 sFolderClosing 块内读此标记→mkPin 钉 alpha=0, 灭单帧复显闪。
    if (sFolderClosing) {
        @try {
            UIView *bornLbl = (UIView *)self;
            UIView *bornIV = MKIconViewForLabel(bornLbl);
            if (bornIV && MKIsInsideFolderThumb((SBIconView *)bornIV)) {
                objc_setAssociatedObject(bornIV, &kMKBornInCloseFolderKey, @(YES), OBJC_ASSOCIATION_RETAIN_NONATOMIC);
            }
        } @catch (NSException *e) {}
    }
    @try {
        ...
        UIView *realIV = MKIconViewForLabel(lbl); // 当前真实属主（superview 链几何反解）
        if (realIV && realIV != storedOwner) {
            ...
            lbl.alpha = 0.0f;
            lbl.layer.opacity = 0.0f;
            if (sDebugLog) RDLog(@"WARNING DOCK-MISMATCH: cls=%@ oldOwner=%p realIV=%p cleared+hidden",
                                 NSStringFromClass([lbl class]), storedOwner, realIV);
        }
    } @catch (NSException *e) {}
}

// v2.0.40: 堵「关文件夹 settle 末拍名称闪现」的 alpha 回弹入口。v2.0.67: 扩展为关窗期间 folder-internal 全部 label(含非运行 app) blanket 钉 alpha=0(绕缓存 bid)。v2.0.68: 方案 D —— 身份判定由 superview 链改为「关窗 ARM 枚举的 folder 内层迷你图标 SBIconView 指针集合」查归属(指针身份最稳, 灭 v2.0.67 缩回重父化漏网), 灭关窗 ~1s 名称闪现。
...
static void MKSetIconLabelAlphaHook(id self, SEL _cmd, CGFloat a) {
    ...
    @try {
        // v2.0.69 方案 E：恒定身份 = bundleID。关窗 ARM 时枚举 folder 内层迷你图标收 bundleID 集(sFolderClosePinBids)；
        // 关窗期直读 [[self icon] applicationBundleID](经 MKGetCachedBid) ∈ 集合 即钉藏。
        // 抗 v2.0.67 的 superview 链失效(重父化到 SBIconListView)、抗 v2.0.68 的指针集失效(同 App 迷你↔主屏格换 SBIconView 对象)。
        // 同时保留 v2.0.68 指针集作 OR 兜底；双信号任一命中即钉。
        if (sFolderClosing) {
            // v2.0.70 方案 F —— 恒定身份 = 实时 [[self icon] applicationBundleID]，绕开 MKGetCachedBid 缓存丢失(关窗期 icon 重设/回收致 bid 解析为空→漏网 89 条)。
            // 优先用持久 cache(sFolderClosePinBids, folder 完整在窗口时枚举); 其次现场实时 bid; 指针集(sFolderClosePinViews, 方案 D)作 OR 兜底;
            // icon==nil 兜底：关窗动画中图标已拆、无法解析 bid 的 label 直接钉藏(宁可错杀, 关窗期本就要隐运行 App 名, 且 sFolderClosing 短时窗结束即恢复)。
            id mkIc = nil;
            @try { mkIc = [(SBIconView *)self icon]; } @catch (NSException *e) {}
            NSString *mkLiveBid = nil;
            if (mkIc && [mkIc respondsToSelector:@selector(applicationBundleID)]) {
                mkLiveBid = [mkIc applicationBundleID];
            }
            if (!mkLiveBid.length) {
                mkLiveBid = MKGetCachedBid((SBIconView *)self); // 缓存兜底(兼容 v2.0.69)
            }
            if (mkLiveBid.length && sFolderClosePinBids && [sFolderClosePinBids containsObject:mkLiveBid]) {
                mkPin = YES;
            } else if (!mkPin && sFolderClosePinViews) {
                NSValue *mkKey = [NSValue valueWithNonretainedObject:(id)self];
                if ([sFolderClosePinViews containsObject:mkKey]) mkPin = YES;
            } else if (!mkIc) {
            ...
            // v2.0.75 ①: 诞生期钉的标记(抗 SBIconView 重建/重父化)→ 直接 mkPin, 覆盖 superview链/pointer集/bid 三法全失效的漏网
            if (!mkPin && objc_getAssociatedObject((UIView *)self, &kMKBornInCloseFolderKey)) {
                mkPin = YES;
            }
        ...
        BOOL inOpenFolder = (!sFolderClosing && MKIsIconInFolder((UIView *)self));   // v2.0.77：②探针 + 与另三 hook 同款门控
        MKOpenFixProbe((UIView *)self, inOpenFolder, __func__);   // v2.0.77 ②探针
        // v2.0.66-diag / v2.0.67-verify: 关窗内 iOS 经 setIconLabelAlpha:>0 复显 label 都记(REVEAL-ATTEMPT)，
    ...
    // v2.0.67: 关窗 folder-internal 钉藏期间不重显 β点(它随 label 区域一起被钉藏)，关完 sFolderClosing 解除后由 MKBetaReconcile 正常恢复；否则 β点会在关窗内闪一下
    if ([MKConfig sharedConfig].keepBetaDot && !mkPin) {
    ...
    MKSafe(^{
        ...
        if (MKIsFolderIcon((SBIconView *)self)) {
            ...
            if (!indicator) {
                ...
                if (sDebugLog) RDLog(@"FICON-CREATE v2.0.35: %@ rep=%@ fixed=%d container=%@ frame=%@", fBid, rep, fixedColor, fContainerCls, NSStringFromCGRect(indicatorFrame));
        ...
        if (!indicator) {
            ...
            if (sDebugLog) RDLog(@"Indicator CREATE v2.0.78: %@ shape=%d animate=%d label=%@",
        ...
        for (UIWindow *window in windows) {
            ...
            while (stack.count > 0) {
                ...
                if ([current isKindOfClass:MKSBIconViewClass()]) {
                    ...
                    if ([MKConfig sharedConfig].keepBetaDot) MKBetaReconcile((SBIconView *)current); // v2.0.34: re-show hidden beta when ON
                }
                for (UIView *child in current.subviews) {
                    [stack addObject:child];
                }
...
static void MKFadeInFolderIndicatorIfClosing(UIView *ind) {
    if (!ind) return;
    if (!sFolderClosing) return;       // 仅关文件夹过渡期淡入
...
// v2.0.68 方案 D + v2.0.69 方案 E 辅助 —— 枚举 folder 子树内全部 SBIconView(迷你 App 图标)指针，包装成 NSValue(nonretained) 集合。
// 仅取图标视图、排除文件夹视图自身。指针身份抗重父化/缓存失效。
static NSSet *MKMkFolderClosePinSet(UIView *fv) {
    NSMutableSet *s = [NSMutableSet set];
    if (!fv) return s;
    Class ivCls = MKSBIconViewClass();
    NSMutableArray *st = [NSMutableArray arrayWithObject:fv];
    while (st.count > 0) {
        UIView *cur = [st lastObject]; [st removeLastObject];
        if (ivCls && [cur isKindOfClass:ivCls] && cur != fv) {
            [s addObject:[NSValue valueWithNonretainedObject:(id)cur]];
        }
        [st addObjectsFromArray:cur.subviews];
    }
    return s;
}
// v2.0.69 方案 E 辅助 —— 同上述 BFS，收 folder 内层 App 的 bundleID 集合(恒定身份，抗迷你↔主屏格换 SBIconView 对象)。关窗期 hook 直读 [[self icon] applicationBundleID] ∈ 集合 即钉藏。
static NSSet *MKMkFolderClosePinBids(UIView *fv) {
...
static void MKArmFolderCloseGuard(NSSet *pinViews, NSSet *pinBids) {
    sFolderClosing = YES;
    sFolderClosePinViews = [pinViews copy];   // v2.0.68 方案 D —— 指针集(抗重父化)
    sFolderClosePinBids   = [pinBids copy];     // v2.0.69 方案 E —— bundleID 集(抗迷你↔主屏格换对象)
    sFolderCloseDiag = 0;
    ...
    sFlashWindow = YES; sFlashStart = [NSDate date].timeIntervalSince1970; sFlashLogCount = 0; sRevealLogCount = 0; sThumbLogCount = 0;
        ...
        if (sFolderCloseGuard) {
            ...
            dispatch_source_set_event_handler(sFolderCloseGuard, ^{
                ...
                if (fcTicks >= 150) {
                    ...
                    sFolderClosing = NO;
                    sFolderClosePinViews = nil;  // v2.0.68 方案 D 关窗结束清集合
                    sFolderClosePinBids   = nil;  // v2.0.69 方案 E 清 bundleID 集
                    sFolderCloseArmCacheBids = nil;  // v2.0.70 方案 F 清持久 cache
                    sFlashWindow = NO; sFlashStart = 0;
                    // v2.0.75: 清理诞生期标记, 防跨次关窗残留(回收复用的 home 图标被误钉)
                    {
                        Class ivClsR = MKSBIconViewClass();
                        NSArray *winsR = [UIApplication sharedApplication].windows;
                        for (UIWindow *wR in winsR) {
                            NSMutableArray *stkR = [NSMutableArray arrayWithObject:wR];
                            while (stkR.count > 0) {
                                UIView *curR = [stkR lastObject]; [stkR removeLastObject];
                                if (ivClsR && [curR isKindOfClass:ivClsR]) {
                                    objc_setAssociatedObject((UIView *)curR, &kMKBornInCloseFolderKey, nil, OBJC_ASSOCIATION_ASSIGN);
                                }
                                [stkR addObjectsFromArray:curR.subviews];
                            }
                        }
                    }
                                        if (sDebugLog) {
        ...
        } else { sFolderClosing = NO; sFolderClosePinViews = nil; sFolderClosePinBids = nil; sFolderCloseArmCacheBids = nil; if (sProbeLog) RDLog(@"FOLDER-CLOSE-ARM on=0 (guard-alloc-fail)"); }}
...
- (void)viewWillDisappear:(BOOL)animated {
    ...
    if (sFolderOpen) {
        ...
        // v2.0.68 方案 D + v2.0.69 方案 E —— 此刻文件夹尚在窗口、子树有效，枚举其内部迷你图标：指针集(方案 D) + bundleID 集(方案 E)。
        NSSet *mkPinSet  = MKMkFolderClosePinSet([(UIViewController *)self view]);
        NSSet *mkPinBids = sFolderCloseArmCacheBids ? sFolderCloseArmCacheBids : MKMkFolderClosePinBids([(UIViewController *)self view]);  // v2.0.70 方案 F：优先用持久 cache，空则现场枚举兜底
        MKArmFolderCloseGuard(mkPinSet, mkPinBids);
    }
}
%end
%hook SBFolderView
...
- (void)didMoveToWindow {
    ...
    if (me.window && sInitDone) {
        ...
        sFolderOpen = YES;
        sFolderClosing = NO; sFolderClosePinViews = nil; sFolderClosePinBids = nil;  // v2.0.3/v2.0.68/v2.0.69: 开文件夹即退出关动画窗口并清双集，避免上一轮关闭残留误触发
        sFolderCloseArmCacheBids = [MKMkFolderClosePinBids(me) copy];  // v2.0.70 方案 F：folder 完整在窗口时枚举内层 App bundleID 集作持久 cache（关闭时 ARM 直接用，绕开关窗子树已在拆的枚举不全）
        if (sProbeLog) RDLog(@"FOLDER-PIN bids=%lu views=%lu", (unsigned long)(sFolderCloseArmCacheBids ? sFolderCloseArmCacheBids.count : 0), (unsigned long)0);
        ...
        sFolderOpen = NO;
        // v2.0.68 方案 D + v2.0.69 方案 E —— me 即正脱离窗口的 SBFolderView，枚举其子树迷你图标：指针集(方案 D) + bundleID 集(方案 E)(见 viewWillDisappear 说明，作后备枚举点)
        NSSet *mkPinSet2  = MKMkFolderClosePinSet(me);
        NSSet *mkPinBids2 = sFolderCloseArmCacheBids ? sFolderCloseArmCacheBids : MKMkFolderClosePinBids(me);  // v2.0.70 方案 F：优先用持久 cache
        MKArmFolderCloseGuard(mkPinSet2, mkPinBids2);  // v2.0.8: 关闭保护提前到关闭起始武装（见函数注释）
