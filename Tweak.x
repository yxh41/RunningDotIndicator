...
static void MKEnsureBetaOnIconView(UIView *iconView, UIView *dot) {
    ...
    if (from != iconView) {
        CGRect df = [dot frame];
        CGRect dfInIcon = from ? [iconView convertRect:df fromView:from] : df;
        [dot removeFromSuperview];
        [iconView addSubview:dot];
        // v2.0.55: 相对 iconView 定位取代绝对 center —— 小黄点随图标几何（锁屏/解锁缩放、翻页重建）跟随，不再钉死错位
        dot.translatesAutoresizingMaskIntoConstraints = YES;
        dot.autoresizingMask = UIViewAutoresizingFlexibleRightMargin | UIViewAutoresizingFlexibleBottomMargin; // 钉左上角（beta 点原生锚点）
        dot.frame = dfInIcon;
    }
...
static void MKUpdate(SBIconView *self) {
    MKSafe(^{
        ...
        if (!indicator) {
            ...
            if (sDebugLog) RDLog(@"Indicator CREATE v2.0.55: %@ shape=%d animate=%d label=%@",
