...
// v2.0.40: 堵「关文件夹 settle 末拍名称闪现」的 alpha 回弹入口。v2.0.67: 扩展为关窗期间 folder-internal 全部 label(含非运行 app) blanket 钉 alpha=0(稳健身份绕过缓存 bid)，灭关窗 ~1s 名称闪现。
...
static void MKSetIconLabelAlphaHook(id self, SEL _cmd, CGFloat a) {
    BOOL mkPin = NO;   // v2.0.67: 关窗期间 folder-internal label 区域钉 alpha=0（灭关窗 ~1s 名称闪现）
    @try {
        // v2.0.67: 稳健身份——superview 链找 SBFloatyFolderScrollView（关窗时文件夹内层图标容器），
        // 绕过 MKGetCachedBid 缓存(关窗转场里缓存 bid 翻转成 0 → 旧判定放行 → 闪)。零 BFS、零分配。
        if (sFolderClosing) {
            UIView *sv = [self superview];
            while (sv) {
                const char *n = class_getName(object_getClass(sv));
                if (strcmp(n, "SBFloatyFolderScrollView") == 0) { mkPin = YES; break; }
                sv = sv.superview;
            }
        }
...
static void MKUpdate(SBIconView *self) {
    MKSafe(^{
        ...
        if (!indicator) {
            ...
            if (sDebugLog) RDLog(@"Indicator CREATE v2.0.67: %@ shape=%d animate=%d label=%@",
