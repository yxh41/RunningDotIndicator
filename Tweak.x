    ...
    MKSafe(^{
        ...
        if (MKIsFolderIcon((SBIconView *)self)) {
            ...
            if (!indicator) {
                ...
                if (sDebugLog) RDLog(@"FICON-CREATE v2.0.13: %@ rep=%@ mode=%ld fixed=%d container=%@ frame=%@", fBid, rep, (long)fCfg.folderIndicatorMode, fixedColor, fContainerCls, NSStringFromCGRect(indicatorFrame));
            } else {
        ...
        if (!indicator) {
            ...
            if (sDebugLog) RDLog(@"Indicator CREATE v2.0.13: %@ shape=%d animate=%d label=%@",
...
// v2.0.13: 拦截 SBIconView 的「图标名 alpha」专用 setter，根治关普通文件夹缩回末尾名称闪现(第④点)。
...
// 修法: 本 bid 当前有指示器(∈sHiddenBids)时, 直接 return 不调 orig, 维持 label 隐藏态, 掐断缩回动画带名。
// 仅对运行 App 生效(其余走 %orig 透传, ABi 安全: 我们只 return 或 %orig 透传, 不基于参数 a 做业务判断)。
- (void)setIconLabelAlpha:(CGFloat)a {
    ...
    if (b.length && sHiddenBids && [sHiddenBids containsObject:b]) {
        if (sDebugLog) RDLog(@"SET-ICONLABELALPHA: skip bid=%@ (keep hidden)", b);
        return;
    }
    %orig;
}
