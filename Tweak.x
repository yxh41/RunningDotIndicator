...
// v2.0.66.16: 递归数视图子树内可见的 name label 数(用于 dock 串名「多 label」兜底判据)
static NSInteger MKCountVisibleNameLabels(UIView *view) {
    if (!view) return 0;
    NSInteger c = 0;
    if (!view.hidden && view.alpha > 0.01f && !CGRectIsEmpty(view.bounds)) {
        NSString *cn = NSStringFromClass([view class]);
        if ([view isKindOfClass:[UILabel class]] ||
            [cn containsString:@"Label"] || [cn containsString:@"Legibility"] ||
            [cn containsString:@"IconLabel"] || [cn containsString:@"TitleLabel"]) {
            c++;
        }
    }
    for (UIView *sv in view.subviews) {
        c += MKCountVisibleNameLabels(sv);
    }
    return c;
}
...
static void MKStrayNameProbe(UIView *label) {
    ...
    @try {
        ...
        // Dock: 仅当真父 SBIconView 自身在 Dock 容器内(说明它真挂在 Dock 槽), 且存在 bid/文字不符, 才视为串名
        // —— 排除「底部主屏图标 label 几何压入 Dock frame 区」的误报(rd_log(144) 18 条 DOCK-OVL 全为此类)
        NSString *lblBid = MKLabelToBid(label);   // v2.0.66.16: 提前取, dock 判据要用
        BOOL ownerInDock = NO;
        UIView *dock = MKFindDockView();
        if (dock && owner) { UIView *ap = owner; while (ap) { if (ap == dock) { ownerInDock = YES; break; } ap = ap.superview; } }
        // v2.0.66.16: dock 设计无 name label(藏名+指示器), dock 串名 = 主屏运行中 app B 的 SBIconView 被 iOS 回收复用去显示 dock 槽 A, B 的 label 跟着被带过去(v1.6.99 recycle 清理 MKAssocLabelBid(oldLabel,nil) 的缺口)。
        // 用户实证现象: dock 槽 A 自己名没错, 但多出了 B 的名(重复显示一份)。
        // 判据①硬不等: label 自身关联 bid(lblBid) != dock 槽当前 icon bid(parentBid) → B≠A 串名(覆盖 lblBid 未被清的常见回收场景)。
        // 判据②多 label 兜底: dock 槽 SBIconView 下挂 >1 个可见 name label(A 名+B 名同现) → 串名(覆盖 lblBid 被清=nil 的极端场景, 不依赖读文字)。
        BOOL dockStrayHard = (ownerInDock && lblBid && parentBid && ![lblBid isEqualToString:parentBid]);
        BOOL dockStrayMulti = (ownerInDock && MKCountVisibleNameLabels(owner) > 1);
        BOOL dockStray = (owner && (dockStrayHard || dockStrayMulti || bidMismatch || textMismatch));
