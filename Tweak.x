...
// ====================================================================
// v2.0.66.10: 泛化「标签落入外来源容器」串名探针 (STRAY-NAME)，覆盖 Dock 与负一屏 Today/Widget 宿主
// --------------------------------------------------------------------
...
// v2.0.66.10: 判定 label 父链是否落在「已知非标准图标槽」容器；命中返回 ctx 串(否则 nil)。
...
// v2.0.66.10: 泛化串名探针 —— 任意可见 label 落入「非标准图标槽」容器即记录。
// Dock 运行中 app 名本就显示(跳过); 负一屏 Today/Widget 宿主本不应含图标名 label，任一可见即可疑。
static void MKStrayNameProbe(UIView *label) {
    if (!label) return;
    @try {
        NSString *ctx = MKForeignContainerCtx(label);
        if (!ctx) return;                              // 仅在已知「非标准图标槽」容器里查(主屏标准槽跳过)
        NSTimeInterval now = [NSDate timeIntervalSinceReferenceDate];
        if (label == sLastStrayLabel && now - sLastStrayLog < 30.0) return; // 同 label 节流
        UIView *slot = MKIconViewForLabel(label);      // 几何反解下方 SBIconView(可能 nil)
...
#pragma clang diagnostic pop
        ...
        RDLog(@"STRAY-NAME: ctx=%@ slotBid=%@ labelText=%@ lblBid=%@ "
...
static void MKSetHiddenHook(id self, SEL _cmd, BOOL hidden) {
    @try {
        ...
        if (!hidden) MKStrayNameProbe((UIView *)self); // v2.0.66.10: 泛化外来源容器串名探针(含负一屏)
...
static void MKSetAlphaHook(id self, SEL _cmd, CGFloat a) {
    @try {
        ...
        if (a > 0.0f) MKStrayNameProbe((UIView *)self); // v2.0.66.10: 泛化外来源容器串名探针(含负一屏)
        NSString *useBid = nil; BOOL mapOnly = NO;
    ...
    MKSafe(^{
        ...
        if (MKIsFolderIcon((SBIconView *)self)) {
            ...
            if (!indicator) {
                ...
                if (sDebugLog) RDLog(@"FICON-CREATE v2.0.66.10: %@ rep=%@ fixed=%d container=%@ frame=%@", fBid, rep, fixedColor, fContainerCls, NSStringFromCGRect(indicatorFrame));
        ...
        if (!indicator) {
            ...
            if (sDebugLog) RDLog(@"Indicator CREATE v2.0.66.10: %@ shape=%d animate=%d label=%@",
