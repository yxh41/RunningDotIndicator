    ...
    MKSafe(^{
        ...
        if (MKIsFolderIcon((SBIconView *)self)) {
            ...
            if (!indicator) {
                ...
                if (sDebugLog) RDLog(@"FICON-CREATE v2.0.33: %@ rep=%@ fixed=%d container=%@ frame=%@", fBid, rep, fixedColor, fContainerCls, NSStringFromCGRect(indicatorFrame));
            } else {
        ...
        if (!indicator) {
            ...
            if (sDebugLog) RDLog(@"Indicator CREATE v2.0.33: %@ shape=%d animate=%d label=%@",
...
- (void)layoutSubviews {
    ...
    if (mustHide) {   // v2.0.12: 撤销 v2.0.9 关合窗口内文件夹内 icon 让步原生(无条件强制藏名), 根治 sub-16ms settle 单帧闪现(第④点残留)。详见 MKSetHiddenHook 同款注释。
        ...
        while (mkSub.count > 0) {
            ...
            if (mkIsL && !mkS.hidden && mkS.alpha > 0.0f && !([MKConfig sharedConfig].keepBetaDot && MKBetaClass(mkS))) {  // v2.0.33: 保留小黄点开关开时，白名单 beta 类视图（小黄点是 SBIconView 兄弟节点，不在 label 子树，脱钩找不到→靠这里保住）
