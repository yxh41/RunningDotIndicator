...
static void MKSetIconLabelAlphaHook(id self, SEL _cmd, CGFloat a) {
    ...
    @try {
        ...
        if (sFolderClosing) {
            if (sFolderClosePinBids) {
                NSString *mkBid = MKGetCachedBid((SBIconView *)self);
                if (mkBid.length && [sFolderClosePinBids containsObject:mkBid]) mkPin = YES;
            }
            if (!mkPin && sFolderClosePinViews) {
...
static void MKUpdate(SBIconView *self) {
    MKSafe(^{
        ...
        if (!indicator) {
            ...
            if (sDebugLog) RDLog(@"Indicator CREATE v2.0.69: %@ shape=%d animate=%d label=%@",
...
static void MKArmFolderCloseGuard(NSSet *pinViews, NSSet *pinBids) {
    ...
    sFlashWindow = YES; sFlashStart = [NSDate date].timeIntervalSince1970; sFlashLogCount = 0; sRevealLogCount = 0; sThumbLogCount = 0;
        ...
        } else { sFolderClosing = NO; sFolderClosePinViews = nil; sFolderClosePinBids = nil; if (sProbeLog) RDLog(@"FOLDER-CLOSE-ARM on=0 (guard-alloc-fail)"); }}
...
- (void)viewWillDisappear:(BOOL)animated {
    ...
    if (sFolderOpen) {
        ...
        NSSet *mkPinBids = MKMkFolderClosePinBids([(UIViewController *)self view]);
...
- (void)didMoveToWindow {
    ...
    if (me.window && sInitDone) {
        ...
        NSSet *mkPinBids2 = MKMkFolderClosePinBids(me);
