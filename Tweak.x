...
static NSSet *sFolderClosePinViews = nil;  // v2.0.68: 方案 D —— 关窗 ARM 时枚举的 folder 内层迷你图标 SBIconView 指针集合(NSValue nonretained 包装)，关窗期 hook 仅查指针归属，不依赖 superview 链/icon.location/缓存 bid(皆可能关窗动画中失效)，指针身份最稳。
...
static void MKArmFolderCloseGuard(NSSet *pinViews);  // v2.0.8/v2.0.68: 关闭保护(缩回动画进行中即武装 guard)；v2.0.68 方案 D 改收「folder 内层迷你图标指针集合」作关窗期身份源；SBFolderView/-didMoveToWindow 与 SBFolderController/-viewWillDisappear 共调用
...
static void MKSetIconLabelAlphaHook(id self, SEL _cmd, CGFloat a) {
    BOOL mkPin = NO;   // v2.0.68: 方案 D —— 关窗 folder-internal label 区域钉 alpha=0（灭关窗 ~1s 名称闪现）
    @try {
        // v2.0.68: 方案 D 稳健身份——查 self 指针是否 ∈ 关窗 ARM 时枚举的 folder 内层迷你图标集合(sFolderClosePinViews)，
        // 绕过 v2.0.67 的 superview 链(关窗缩回时 iOS 把 label 重父化到 SBIconListView → 链上找不到 SBFloatyFolderScrollView → 漏)、
        // 也绕过 MKGetCachedBid 缓存(关窗转场里缓存 bid 翻转成 0 → 放行 → 闪)。指针身份在重父化/location 翻转/缓存失效下都不变，最稳。零 BFS、零分配(NSValue 比较)。
        if (sFolderClosing && sFolderClosePinViews) {
...
static void MKUpdate(SBIconView *self) {
    MKSafe(^{
        ...
        if (!indicator) {
            ...
            if (sDebugLog) RDLog(@"Indicator CREATE v2.0.68: %@ shape=%d animate=%d label=%@",
...
// v2.0.68: 方案 D 辅助 —— 枚举文件夹视图子树内全部 SBIconView(迷你 App 图标)指针，包装成 NSValue(nonretained) 集合。
// 仅取图标视图、排除文件夹视图自身。指针身份在关窗重父化/location 翻转/缓存失效下都不变，作最稳的关窗期身份源。
...
static void MKArmFolderCloseGuard(NSSet *pinViews) {
    sFolderClosing = YES;
    sFolderClosePinViews = [pinViews copy];   // v2.0.68: 方案 D —— 收下关窗 ARM 枚举的 folder 内层迷你图标指针集合(关窗期 hook 查指针归属)
    sFolderCloseDiag = 0;
    ...
    sFlashWindow = YES; sFlashStart = [NSDate date].timeIntervalSince1970; sFlashLogCount = 0; sRevealLogCount = 0; sThumbLogCount = 0;
        ...
        } else { sFolderClosing = NO; sFolderClosePinViews = nil; if (sProbeLog) RDLog(@"FOLDER-CLOSE-ARM on=0 (guard-alloc-fail)"); }}
...
- (void)didMoveToWindow {
    ...
    if (me.window && sInitDone) {
        ...
        sFolderOpen = YES;
        sFolderClosing = NO; sFolderClosePinViews = nil;  // v2.0.3: 开文件夹即退出关动画窗口，避免上一轮关闭的 sFolderClosing 残留误触发诊断
        ...
        sFolderOpen = NO;
        // v2.0.68: 方案 D —— me 即正脱离窗口的 SBFolderView，枚举其子树迷你图标指针(见 viewWillDisappear 说明，作后备枚举点)
        NSSet *mkPinSet2 = MKMkFolderClosePinSet(me);
        MKArmFolderCloseGuard(mkPinSet2);  // v2.0.8: 关闭保护提前到关闭起始武装（见函数注释）
