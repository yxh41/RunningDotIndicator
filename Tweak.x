...
static void *sLastDockMismatchOwner = NULL;
static void *sLastDockMismatchSlot  = NULL;
...
static void MKDockNameMismatchProbe(UIView *label) {
    ...
    @try {
        ...
        if (owner == (UIView *)sLastDockMismatchOwner &&
            slot  == (UIView *)sLastDockMismatchSlot &&
            now - sLastDockMismatchLog < 30.0) return; // 同组合节流
        sLastDockMismatchLog = now;
        sLastDockMismatchOwner = (void *)owner;
        sLastDockMismatchSlot  = (void *)slot;
    ...
    MKSafe(^{
        ...
        if (MKIsFolderIcon((SBIconView *)self)) {
            ...
            if (!indicator) {
                ...
                if (sDebugLog) RDLog(@"FICON-CREATE v2.0.66.7: %@ rep=%@ fixed=%d container=%@ frame=%@", fBid, rep, fixedColor, fContainerCls, NSStringFromCGRect(indicatorFrame));
        ...
        if (!indicator) {
            ...
            if (sDebugLog) RDLog(@"Indicator CREATE v2.0.66.7: %@ shape=%d animate=%d label=%@",
