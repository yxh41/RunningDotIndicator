...
static void MKSetIconLabelAlphaHook(id self, SEL _cmd, CGFloat a) {
    ...
    @try {
        ...
        // v2.0.82 ②: 打开文件夹内部运行 App 名称常驻藏名(灭打开态/关窗后的名称周期复显闪现)。
        ...
        //   运行判定改用权威 MKIsAppRunning(bid)(与 sHiddenBids 清理 L344 同款, 不依赖易失集合, 零成本 NSSet 查表)。
        ...
        if (!mkPin && sFolderOpen && MKIconInClosingFolderView((UIView *)self)) {
            ...
            if (obid.length && MKIsAppRunning(obid)) {
                mkPin = YES;
            }
        }
...
static void MKUpdate(SBIconView *self) {
    MKSafe(^{
        ...
        if (!indicator) {
            ...
            if (sDebugLog) RDLog(@"Indicator CREATE v2.0.82: %@ shape=%d animate=%d label=%@",
