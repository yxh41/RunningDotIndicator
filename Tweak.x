...
static void MKUpdate(SBIconView *self) {
    MKSafe(^{
        ...
        if (!indicator) {
            ...
            if (sDebugLog) RDLog(@"Indicator CREATE v2.0.73: %@ shape=%d animate=%d label=%@",
...
%ctor {
    ...
    // v2.0.73: 修 v2.0.72 两回归(误藏指示器→文件夹内运行点消失; hidden=YES→打开闪一下) —— 改「仅压 alpha/opacity 钉藏名称 label、不碰 hidden/指示器」。根因/真修同 v2.0.72。
