...
static void MKStrayNameProbe(UIView *label) {
    ...
    @try {
        ...
        if (!owner) return;                                        // 不是图标名 label
        // v2.0.66.13: 改用工程统一 MKGetCachedBid(取代自写 MKBidOfIconView, 修 parentBid 全 nil)
        NSString *parentBid = MKGetCachedBid((SBIconView *)owner);
        ...
        if (dock) { UIView *ap = owner; while (ap) { if (ap == dock) { ownerInDock = YES; break; } ap = ap.superview; } }
        BOOL dockStray = (ownerInDock && (bidMismatch || textMismatch));
        ...
        if (!foreignStray && !dockStray && !negOverlap && !misAttached && !bidMismatch && !textMismatch && !nonHomeStray) return;
        ...
        RDLog(@"STRAY-NAME: ctx=%@ parentBid=%@ intendedBid=%@ labelText=%@ lblBid=%@ chain=%@ labelF=(%.0f,%.0f,%.0f,%.0f)",
              finalCtx,
              (parentBid ? parentBid : @"?"),
