...
static void MKEnsureBetaOnIconView(UIView *iconView, UIView *dot) {
    ...
    if (from != iconView) {
        CGPoint c = (from && iconView) ? [from convertPoint:dot.center toView:iconView] : dot.center; // v2.0.59: 父视图可能未挂层级 → 退化用 dot.center（label≈iconView 几何，位置近似正确，且每帧 BFS 会自校正）
...
static void MKDetachBetaOnce(UIView *iconView) {
    ...
    // v2.0.59: 落实 v2.0.50 意图 —— 终于删掉 !label.superview 早退。那会让小黄点困在被藏的 label 内、
    // 直到下次 layout/swipe 才脱离（即「解锁后小黄点消失、滑屏才出来」真凶之一）。只要 label 存在就在其
    // 子树找点（MKFindBetaInLabel 不依赖 superview）；label 无 superview 时坐标退化由 MKEnsureBetaOnIconView 兜底。
    if (!label) return;
...
static void MKUpdate(SBIconView *self) {
    MKSafe(^{
        ...
        if (!indicator) {
            ...
            if (sDebugLog) RDLog(@"Indicator CREATE v2.0.59: %@ shape=%d animate=%d label=%@",
