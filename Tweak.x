...
// v2.0.60-dbg: 枚举「含 beta 小黄点的运行 App」label 子树 —— 定位「名称文本子视图」确切类名与 beta 配件布局，
// 为「只藏文本、保留 beta 点」修复(灭偏上+灭解锁消失)提供证据。每 label 仅打一次(assoc 标记)，
// 且只在确实发现 MKBetaClass 子视图时才落盘(否则每帧重试直到 beta 点出现)。
static void MKDebugDumpBetaLabel(UIView *label) {
    ...
    if (!mkHasBeta) return; // 此帧无 beta 点 → 不标记，下帧再探
    ...
    NSMutableString *mkS = [NSMutableString stringWithFormat:
        @"BETA-LABEL-DUMP cls=%@ frame=%@ center=%@ hidden=%d alpha=%.2f subviews=%lu",
        NSStringFromClass([label class]), NSStringFromCGRect(label.frame),
...
static void MKDetachBetaOnce(UIView *iconView) {
    ...
    MKDebugDumpBetaLabel(label); // v2.0.60-dbg: 枚举含 beta 点的 label 子树(诊断)
...
static void MKUpdate(SBIconView *self) {
    MKSafe(^{
        ...
        if (!indicator) {
            ...
            if (sDebugLog) RDLog(@"Indicator CREATE v2.0.60: %@ shape=%d animate=%d label=%@",
