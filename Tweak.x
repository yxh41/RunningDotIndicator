...
static void MKDetachBetaOnce(UIView *iconView) {
    ...
    MKDebugDumpBetaLabel(label, MKGetCachedBid((SBIconView *)iconView)); // v2.0.61-dbg: 枚举运行 App label 子树(诊断)
...
static void MKUpdate(SBIconView *self) {
    MKSafe(^{
        ...
        if (!indicator) {
            ...
            if (sDebugLog) RDLog(@"Indicator CREATE v2.0.61: %@ shape=%d animate=%d label=%@",
