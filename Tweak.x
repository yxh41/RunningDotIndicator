...
static void MKUpdate(SBIconView *self) {
    MKSafe(^{
        ...
        if (!indicator) {
            ...
            if (sDebugLog) RDLog(@"Indicator CREATE v2.0.70: %@ shape=%d animate=%d label=%@",
...
// v2.0.72: 关文件夹缩回时, 文件夹缩略图(SBFolderIconImageView)子树里的迷你 app 图标名称 label
// (SBIconLegibilityLabelView) 与我们的运行点(kDotTag / 类名含 Indicator) 会随缩略图【活 view 飞回】≈1s 闪现
// (v2.0.71 rd_log(120) 实证: THUMB-CHILD 链 SBFolderIconImageView→_SBIconGridWrapperView→SBIconView→
//  SBIconLegibilityLabelView, 与主屏【同属 SBIconView 体系】(推翻旧注释"独立 view 非 SBIconView");
//  SNAP-PRE=0 → 关窗快照不走 snapshotViewAfterScreenUpdates: 即不是烘焙 bitmap, 系活 view 泄漏)。
// 此前 MKProbeFolderThumb 只 dump 不藏, MKFadeInFolderIndicatorIfClosing 仅在新建议/重定位时淡入 —— 关窗期不主动钉藏已存在的迷你 label。
// 本函数每帧(关窗窗内)递归钉藏这些后代: 直接 hidden=YES/alpha=0/layer.opacity=0, 不写入任何持久 pin 缓存
// (sFolderClosePinBids / sFolderCloseArmCacheBids), 关窗结束即释放, 由 iOS 自行恢复稳态(文件夹缩略图本就不显示名称)。
// 范围限定: 仅传入 folderIcon 子树 + 仅 Label/Indicator 类后代 → 主屏/打开文件夹内层/Dock/解锁/keepBetaDot 均不碰, 无侧效。
static void MKHideFolderThumbLabels(UIView *folderIcon) {
    ...
    while (st.count > 0 && depth < 10) {
        ...
        for (UIView *v in st) {
            ...
            BOOL isOurInd = (v.tag == kDotTag) || (strstr(vn, "Indicator") != NULL);
            if (isLabel || isOurInd) {
                if (!v.hidden || v.alpha > 0.0f || v.layer.opacity > 0.0f) {
...
%ctor {
    ...
    // v2.0.72: 落真修 —— 关窗窗内(sFolderClosing)对文件夹缩略图(SBFolderIconImageView)子树递归钉藏迷你 app 名称 label(SBIconLegibilityLabelView)与我们的指示器(kDotTag/"Indicator"), 灭「关非运行文件夹名称/指示器 ~1s 闪现」。
