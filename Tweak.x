...
// ====================================================================
// v2.0.66.9: 几何触发 Dock 串名探针 (DOCK-STRAY-NAME)
// --------------------------------------------------------------------
...
// ====================================================================
static NSTimeInterval sLastDockStrayLog = 0;
static __unsafe_unretained UIView *sLastDockStraySlot = nil;

// Dock 上下文判定: label 祖先链含 SBDock* 类(主屏 SBIconListView/SBIconScrollView 不含此前缀, 不误报)
static BOOL MKLabelInDock(UIView *label) {
    UIView *p = label.superview;
    while (p) {
        NSString *cn = NSStringFromClass([p class]);
        if ([cn hasPrefix:@"SBDock"]) return YES;
        p = p.superview;
    }
    return NO;
}

static void MKDockStrayNameProbe(UIView *label) {
    if (!label) return;
    @try {
        if (!MKLabelInDock(label)) return;             // 仅 Dock(主屏非运行 app 本就显示名称, 不可误报)
        UIView *slot = MKIconViewForLabel(label);      // 几何反解 label 下方 Dock 槽位 SBIconView
        if (!slot) return;
        ...
        if (slotRunning) return;                       // 运行中的 Dock app 本就显示名称 → 正常, 跳过
        // 非运行 Dock 槽却出现可见名称 label → 串名 bug
        NSTimeInterval now = [NSDate timeIntervalSinceReferenceDate];
        if (slot == sLastDockStraySlot && now - sLastDockStrayLog < 30.0) return; // 同槽节流
        sLastDockStrayLog = now;
        sLastDockStraySlot = slot;
...
#pragma clang diagnostic pop
        ...
        NSString *lblBid = MKLabelToBid(label);        // 可能 nil(键已清), 但几何 slotBid 已足够定位
        RDLog(@"DOCK-STRAY-NAME: slotBid=%@ slotRunning=NO labelText=%@ lblBid=%@ "
              @"slotF=(%.0f,%.0f,%.0f,%.0f) labelF=(%.0f,%.0f,%.0f,%.0f)",
              (slotBid ? slotBid : @"?"),
              (txt ? txt : @"?"),
              (lblBid ? lblBid : @"?"),
              slot.frame.origin.x, slot.frame.origin.y, slot.frame.size.width, slot.frame.size.height,
...
// v2.0.66.9: 几何触发 Dock 串名探针（定义见 MKLabelToBid 之后）
static BOOL MKLabelInDock(UIView *label);
static void MKDockStrayNameProbe(UIView *label);
...
static void MKSetHiddenHook(id self, SEL _cmd, BOOL hidden) {
    @try {
        ...
        if (!hidden) MKDockStrayNameProbe((UIView *)self); // v2.0.66.9: 几何触发 Dock 串名探针
...
static void MKSetAlphaHook(id self, SEL _cmd, CGFloat a) {
    @try {
        ...
        if (a > 0.0f) MKDockStrayNameProbe((UIView *)self); // v2.0.66.9: 几何触发 Dock 串名探针
        NSString *useBid = nil; BOOL mapOnly = NO;
    ...
    MKSafe(^{
        ...
        if (MKIsFolderIcon((SBIconView *)self)) {
            ...
            if (!indicator) {
                ...
                if (sDebugLog) RDLog(@"FICON-CREATE v2.0.66.9: %@ rep=%@ fixed=%d container=%@ frame=%@", fBid, rep, fixedColor, fContainerCls, NSStringFromCGRect(indicatorFrame));
        ...
        if (!indicator) {
            ...
            if (sDebugLog) RDLog(@"Indicator CREATE v2.0.66.9: %@ shape=%d animate=%d label=%@",
