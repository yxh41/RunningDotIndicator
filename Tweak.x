...
static void MKScheduleUnlock(void) {
    ...
    dispatch_source_set_event_handler(sUnlockTimer, ^{
        @try {
            ...
            sLocked = NO;
            if (!sContainerToOverlay) return;
            MKRefreshAllIcons();  // 先重定位/重建（解锁后图标视图层级可能被 SpringBoard 重建，overlay 容器随之换新）
            NSArray *all = [sContainerToOverlay.objectEnumerator.allObjects copy];
            for (UIView *ov in all) { ov.hidden = NO; ov.alpha = 0.0f; }  // 刷新后再归零：确保淡入作用在「当前」容器上，避免锚到被替换的旧容器导致硬跳
            [UIView animateWithDuration:0.5 delay:0.0 usingSpringWithDamping:0.7 initialSpringVelocity:0.4
                options:0 animations:^{ for (UIView *ov in all) ov.alpha = 1.0f; }
                completion:nil];
            if (sDebugLog) RDLog(@"UNLOCK(timer): restored all indicators with fade-in");
        } @catch (NSException *e) {
...
static void MKUpdate(SBIconView *self) {
    MKSafe(^{
        ...
        if (!indicator) {
            ...
            if (sDebugLog) RDLog(@"Indicator CREATE v2.0.39: %@ shape=%d animate=%d label=%@",
