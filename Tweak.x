...
static char kMKBetaScannedKey;  // v2.0.30: label 已扫描确认无 beta 子视图 → 跳过每帧 BFS（label 重建即失效）
...
// v2.0.40: 单一受守卫的「解锁复原」入口 —— 所有解锁信号路径（sUnlockTimer 定时器 /
// lockstate=="0" / UIApplicationDidBecomeActive / MKUpdate 时间闸门）都只调用本函数，
// 不再各自持有淡入逻辑，彻底消除多路竞态导致的「空白卡死 / 硬跳 / 重复淡入」。
// 守卫 if(!sLocked)return：本周期第一个触发源跑完后立即置 sLocked=NO，
// 其余触发源进来直接 return，保证每轮解锁只复原一次。
// 顺序严格 refresh-FIRST、capture-AFTER：先 MKRefreshAllIcons() 让 SpringBoard 解锁后
// 重建的容器/overlay 落位，再捕获 all，确保淡入作用在「当前」容器而非被替换的旧僵尸视图。
static void MKUnlockRestore(void) {
    if (!sLocked) return;                          // 已在本轮解锁中复原过 → 跳过（杀多路竞态）
    if (!sContainerToOverlay) return;
...
static void MKScheduleUnlock(void) {
    ...
    dispatch_source_set_event_handler(sUnlockTimer, ^{
        @try {
            if (sUnlockTimer) { dispatch_source_cancel(sUnlockTimer); sUnlockTimer = NULL; }
            if (!sLocked) return;                          // 被其他路径（active/lockstate/闸门）抢先复原 → 不重复
            NSTimeInterval now = [NSDate date].timeIntervalSince1970;
            if (now - sLockAt < 0.7) return;             // 又有更新的锁屏，等下一轮
            MKUnlockRestore();                            // v2.0.40: 统一走单一受守卫入口，不再各自持有淡入逻辑
            if (sDebugLog) RDLog(@"UNLOCK(timer): delegated to MKUnlockRestore");
        } @catch (NSException *e) {
    ...
    @try {
        ...
        if ([n isEqualToString:@"com.apple.springboard.lockcomplete"]) {
            ...
            if ([obj isEqualToString:@"0"]) {
                if (sUnlockTimer) { dispatch_source_cancel(sUnlockTimer); sUnlockTimer = NULL; }
                MKUnlockRestore();  // v2.0.40: 单一受守卫入口（内部置 sLocked=NO + 延迟淡入）
                if (sDebugLog) RDLog(@"UNLOCK(lockstate): delegated to MKUnlockRestore");
                return;
            }
...
static void MKDetachBetaOnce(UIView *iconView) {
    ...
    if (!label || !label.superview) return; // label 未在活层级 → 不动（避免错误坐标转换）
    // 每帧兜底热路径优化 —— 本 label 已扫描确认无 beta 子视图则直接跳过整棵 BFS。
    if ([objc_getAssociatedObject(label, &kMKBetaScannedKey) boolValue]) return;
    UIView *dot = MKFindBetaInLabel(label);
    if (!dot) {
        objc_setAssociatedObject(label, &kMKBetaScannedKey, @YES, OBJC_ASSOCIATION_RETAIN_NONATOMIC);
        return; // 非 beta / 已脱离 → 不动，保留 iconView 上现有脱离点
    }
    // 清掉 iconView 上我们之前脱离的残留孤儿点（若有），保证同一时刻最多 1 个
    for (UIView *sv in [iconView.subviews copy]) {
        if (sv != dot && MKBetaClass(sv)) [sv removeFromSuperview];
    }
    CGPoint c = [label convertPoint:dot.center toView:iconView];
...
static void MKAssocLabelBid(UIView *label, NSString *bid) {
    ...
    if (bid.length) {
        ...
        objc_setAssociatedObject(label, &kMKBetaScannedKey, nil, OBJC_ASSOCIATION_RETAIN_NONATOMIC);  // v2.0.30: 解除藏名关联时同步清 beta 扫描标记（防跨退出→重开残留"无 beta"标记 → 重新被藏）
    }
}
...
static void MKUpdate(SBIconView *self) {
    MKSafe(^{
        ...
        if (sLocked) {
            ...
            if (now - sLockAt > 2.0) {
                if (sUnlockTimer) { dispatch_source_cancel(sUnlockTimer); sUnlockTimer = NULL; }
                MKUnlockRestore();  // v2.0.40: 单一受守卫入口（内部置 sLocked=NO + 延迟淡入）；移除原 sLocked=NO，交给守卫统一处理
        ...
        if (!indicator) {
            ...
            if (sDebugLog) RDLog(@"Indicator CREATE v2.0.40: %@ shape=%d animate=%d label=%@",
...
static void MKArmFolderCloseGuard(void) {
    ...
    sFolderCloseVisDiag = 0;
        ...
        if (sFolderCloseGuard) {
            ...
            dispatch_source_set_timer(sFolderCloseGuard, DISPATCH_TIME_NOW, (int64_t)(0.016 * NSEC_PER_SEC), 0);
            ...
            dispatch_source_set_event_handler(sFolderCloseGuard, ^{
                ...
                if (fcTicks >= 50) {
                    ...
                    sFolderClosing = NO;
                    if (sDebugLog) RDLog(@"FOLDER-CLOSE-ARM on=0 ticks=50");
                }
            });
...
%ctor {
    ...
    %init;
    RDLog(@"======== RDBUILD v2.0.40 (UNLOCK-RACE-FIX: collapse 4 reset triggers - sUnlockTimer / lockstate==\"0\" / UIApplicationDidBecomeActive / MKUpdate gate - into ONE guarded MKUnlockRestore; if(!sLocked)return guarantees exactly one restore per unlock cycle, killing the multi-trigger race that caused blank-stuck / hard-jump / double-fade; strict refresh-FIRST then capture-AFTER (MKRefreshAllIcons before snapshotting overlays) so the fade anchors to the live container SpringBoard rebuilds on unlock, not the stale zombie; removed per-path fade blocks + duplicate sLocked=NO; fade params unchanged 0.9s delay + 0.5s spring)");
    ...
    [[NSNotificationCenter defaultCenter]
        ...
        usingBlock:^(NSNotification *note){
            @try {
                ...
                // v2.0.40: 单一受守卫入口（内部置 sLocked=NO + 延迟淡入）；移除原 sLocked=NO，交给守卫统一处理
                MKUnlockRestore();
                if (sDebugLog) RDLog(@"UNLOCK(active): delegated to MKUnlockRestore");
            } @catch (NSException *e) {}
        }];
