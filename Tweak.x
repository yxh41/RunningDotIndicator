...
static void MKStrayNameProbe(UIView *label) {
    ...
    @try {
        ...
        if (ctx) {
            @try { label.hidden = YES; [label.layer removeAllAnimations]; if (parentBid) MKAssocLabelBid(label, parentBid); } @catch (NSException *e) {}
        }
...
%ctor {
    ...
    if (MKIsDisabled()) {
        RDLog(@"======== RDBUILD-NOTE v2.0.66.23: NEG-CONTAINER 容器级捕获(route A dump-only, 不改显隐零行为变化) —— %hook WGWidgetWrapperView/WGWidgetListFooterView/WGPlatterHeaderContentView/SBTodayView 的 didMoveToWindow+layoutSubviews 扫整棵子树, 找图标名 label/可疑 UILabel 打 NEG-CONTAINER(类+文本+frame+祖先链+isIconName); 接 v2.0.66.22 的 label 级 NEG-EVT, 覆盖其只装图标名 label 类与容器类前缀失配双盲区, 终于抓到负一屏随机名真实类名 ========");
