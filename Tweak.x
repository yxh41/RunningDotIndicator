...
// v2.0.66.28: 决定性捕获(去门控版) —— 直接 hook UILabel.setText:/setAttributedText:。
...
- (void)setText:(NSString *)text {
    %orig;
    if (text && [text length] && ((UIView *)self).window && MKLabelInNegScreen((UIView *)self)) {
...
- (void)setAttributedText:(NSAttributedString *)attributedText {
    %orig;
    if (attributedText && [attributedText length] && ((UIView *)self).window && MKLabelInNegScreen((UIView *)self)) {
...
%ctor {
    ...
    RDLog(@"======== RDBUILD-NOTE v2.0.66.28: NEG-SETTEXT 去门控捕获版 —— 教训 v2.0.66.27 仅靠 sNegScreenVisible 开关 + 文本命中已装 App 名集合 两道假设性门控, 把随机名漏掉(0 命中); 本版去掉这两个假设, 只要 label 位于负一屏容器(SBFFocusIsolationView/SBTodayView/SBDashboard)子树内, 任意 setText:/setAttributedText: 都落 NEG-SETTEXT(类名+文本+isApp标记+完整祖先链); 去重(同 cls+text 只打一次)+上限2000行封顶防日志爆炸; isApp 仅作附加标记不再过滤; 纯观察零行为变化, 待用户发 log 看到随机名(如 loon)对应的 cls+chain 后写精准藏名修复 ========");
