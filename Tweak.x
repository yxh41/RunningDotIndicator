...
static void MKEnsureBetaOnIconView(UIView *iconView, UIView *dot) {
    ...
    if (from != iconView) {
        CGPoint c = [from convertPoint:dot.center toView:iconView];
        [dot removeFromSuperview];
        [iconView addSubview:dot];
        // v2.0.43 那套：dot.center=c 定位（撤 v2.0.55 autoresizing 锚点 hack） —— 小黄点随图标几何（锁屏/解锁缩放、翻页重建）跟随，不再钉死错位
        // v2.0.43: 撤 v2.0.55 autoresizing（改用 dot.center=c）
        // v2.0.43 那套：用 dot.center=c 绝对坐标（label->iconView 转换），撤 v2.0.55 autoresizing 锚点 hack // 钉左上角（beta 点原生锚点）
        dot.center = c;
    }
...
static void MKDetachBetaOnce(UIView *iconView) {
    ...
    // v2.0.50: 不再因 !label.superview 早退 —— 那会让小黄点困在被藏的 label 内、
    // 直到下次 layout/swipe 才脱离（即「滑屏才出来」真凶之一）。只要 label 存在就在其
    // 子树找点（MKFindBetaInLabel 不依赖 superview）；仅当 label 无 superview 时坐标转换退化。
    if (!label || !label.superview) return; // v2.0.43: label 未在活层级 → 不动（避免错误坐标转换/几何瞬变钉歪）
...
static void MKSetHiddenHook(id self, SEL _cmd, BOOL hidden) {
    @try {
        ...
        if ((useBid = MKShouldHideLabel((UIView *)self, bid, &mapOnly))) {   // v2.0.9 的「关闭动画窗口内让步原生」已在 v2.0.12 撤销：关合窗口内文件夹内 label 也强制藏名(不让步原生)，根治 sub-16ms settle 单帧闪现(第④点残留真凶)。证据 rd_log(63): FOLDER-CLOSE-VISIBLE=0 表明无 strobe 互搏，去掉安全。
            ...
            // v2.0.41: 关窗内 iOS 经 setHidden:NO 复显运行 App label 时记一笔(REVEAL-ATTEMPT)
            // 注意：hidden 已在上行置 YES，此处 !hidden 恒为 NO —— 与原逻辑一致（该诊断分支实际不可达）。
            if (sProbeLog && sFlashWindow && !hidden && useBid)
                MKLogRevealAttempt(@"setHidden:", 1.0f, useBid, object_getClass(self));
        }
    } @catch (NSException *e) {}
...
static void MKSetAlphaHook(id self, SEL _cmd, CGFloat a) {
    @try {
        ...
        if ((useBid = MKShouldHideLabel((UIView *)self, bid, &mapOnly))) {   // v2.0.12: 撤销 v2.0.9 关合窗口内让步原生(label 在文件夹内也强制藏名), 根治 sub-16ms settle 单帧闪现。详见 MKSetHiddenHook 同款注释。
            ...
            if (sDebugLog && mapOnly)
                RDLog(@"OVERLAP-GAP: caught via ptr-map cls=%@ bid=%@", object_getClass(self), useBid);
            // v2.0.41: 关窗内 iOS 经 setAlpha:>0 复显运行 App label 时记一笔(REVEAL-ATTEMPT)
            if (sProbeLog && sFlashWindow && inA > 0.0f && useBid)
                MKLogRevealAttempt(@"setAlpha:", inA, useBid, object_getClass(self));
        }
    } @catch (NSException *e) {}
...
static void MKSetIconLabelAlphaHook(id self, SEL _cmd, CGFloat a) {
    @try {
        ...
        // v2.0.41: 关窗内 iOS 经 setIconLabelAlpha:>0 复显运行 App label 时记一笔(REVEAL-ATTEMPT)，须在覆写 a 之前判据
        if (sProbeLog && sFlashWindow && hasBid && a > 0.0f) {
            UIView *rl = MKGetCachedLabel((SBIconView *)self);
            MKLogRevealAttempt(@"setIconLabelAlpha:", a, bid, (rl ? object_getClass(rl) : object_getClass(self)));
        }
...
static void MKUpdate(SBIconView *self) {
    MKSafe(^{
        ...
        if (!indicator) {
            ...
            if (sDebugLog) RDLog(@"Indicator CREATE v2.0.57: %@ shape=%d animate=%d label=%@",
            ...
            if (mkIsL) {
                if (mkKeepBeta && MKBetaClass(mkS)) {
                    ...
                    if (mkSp && mkSp != (UIView *)self) {
                        // v2.0.43 那套：BFS 不强制脱离（脱离只在 MKDetachBetaOnce 走，带 !label.superview 早退）；留空
