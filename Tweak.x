    ...
    @try {
        ...
        if (fctx && !MKIsIconNameLabel(label)) {
            ...
            if (txt && txt.length >= 1 && txt.length <= 40) { // 仅记"文本疑似 App 名", 避免 widget 自身标题/描述/时间噪声爆炸
                ...
                // v2.0.66.19: 修复尝试 —— widget 容器内、在 SBIconView 子树(inIconTree=Y)且关联运行中 App(parentBid ∈ sHiddenBids)的 name label → 强制藏名(走现有藏名逻辑, 安全无盲修风险)。
                // 针对机制(b): SBIconView 回收复用把运行中 App B 的 name label 带进 widget 配置视图。机制(a)(widget 自渲染取错名, inIconTree=N)不触发, 无害, 等 NEG-WIDGET-LABEL 数据确认后再精准修。
                if (inIconTree && parentBid && sHiddenBids && [sHiddenBids containsObject:parentBid]) {
                    @try { label.hidden = YES; [label.layer removeAllAnimations]; MKAssocLabelBid(label, parentBid); } @catch (NSException *e) {}
                }
            }
