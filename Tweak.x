...
// v2.0.20: hideBetaDot（TestFlight 小黄点）功能已整体移除。
// 原因：把小黄点从 SpringBoard 私有 label 子树「拔出来再挂回」对抗其每帧重建，
// 从架构上不稳定（日志见 deep 反复横跳、super 在 iconView/SBFTouchPassThroughView 间乱跳）；
// 唯一稳健替代（只藏名称文字、保留小黄点原位）需侵入刚修稳的 Bug A 藏名不变量，风险过高。
// 按用户决定整功能下线，核心指示器与 Bug A 不受影响。
    ...
    MKSafe(^{
        ...
        if (MKIsFolderIcon((SBIconView *)self)) {
            ...
            if (!indicator) {
                ...
                if (sDebugLog) RDLog(@"FICON-CREATE v2.0.20: %@ rep=%@ mode=%ld fixed=%d container=%@ frame=%@", fBid, rep, (long)fCfg.folderIndicatorMode, fixedColor, fContainerCls, NSStringFromCGRect(indicatorFrame));
            } else {
        ...
        if (!indicator) {
            ...
            if (sDebugLog) RDLog(@"Indicator CREATE v2.0.20: %@ shape=%d animate=%d label=%@",
