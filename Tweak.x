...
static NSString *MKFolderChosenBid(NSArray<NSString*> *bids, NSInteger mode, BOOL fixedColor) {
    ...
    if (pick == 1) {
        NSString *best2 = bids.firstObject;
        NSInteger bestB = MKBadgeForBid(best2);
        for (NSString *b in bids) {
            NSInteger n = MKBadgeForBid(b);
            if (n > bestB) { best2 = b; bestB = n; }
        }
        return best2;
    }
    ...
    MKSafe(^{
        ...
        if (MKIsFolderIcon((SBIconView *)self)) {
            ...
            if (!indicator) {
                ...
                if (sDebugLog) RDLog(@"FICON-CREATE v1.6.94: %@ rep=%@ mode=%ld fixed=%d container=%@ frame=%@", fBid, rep, (long)fCfg.folderIndicatorMode, fixedColor, fContainerCls, NSStringFromCGRect(indicatorFrame));
            } else {
        ...
        if (!indicator) {
            ...
            if (sDebugLog) RDLog(@"Indicator CREATE v1.6.94: %@ shape=%d animate=%d label=%@",
